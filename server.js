...
const AUTO_GAMES = {
  'BR SURVIVAL': { prefix: 'BR Survival', entryFee: 15, prizePool: 220, slots: 20, startOffsetMinutes: 30 },
  'PER KILL MATCHES': { prefix: 'Per Kill Match', entryFee: 11, prizePool: 0, slots: 48, startOffsetMinutes: 35 },
  'CLASH SQUAD 1V1': { prefix: 'Clash Squad 1V1', entryFee: 40, prizePool: 55, slots: 2, startOffsetMinutes: 40 },
  'LONE WOLF 1V1': { prefix: 'Lone Wolf 1V1', entryFee: 40, prizePool: 55, slots: 2, startOffsetMinutes: 45 },
  'CLASH SQUAD 4V4': { prefix: 'Clash Squad 4V4', entryFee: 20, prizePool: 100, slots: 8, startOffsetMinutes: 50 },
  'LONE WOLF 2V2': { prefix: 'Lone Wolf 2V2', entryFee: 25, prizePool: 60, slots: 4, startOffsetMinutes: 55 }
};
const AUTO_GAME_NAMES = Object.keys(AUTO_GAMES);

async function createAutoContest(game, config, startsAt) {
  const latest = await pool.query(
    `SELECT title FROM contests WHERE game=$1 ORDER BY id DESC LIMIT 1`,
    [game]
  );
  ...
  if (match) {
    ...
    const count = await pool.query(
      `SELECT COUNT(*)::int AS count FROM contests WHERE game=$1`,
      [game]
    );
    nextNumber = Number(count.rows[0]?.count || 0) + 1;
  }

  const title = `${config.prefix} #${nextNumber}`;
  const result = await pool.query(
    `INSERT INTO contests
      (game,title,entry_fee,prize_pool,slots,status,starts_at)
     VALUES($1,$2,$3,$4,$5,'upcoming',$6)
     RETURNING *`,
    [game, title, config.entryFee, config.prizePool, config.slots, startsAt]
  );

  return result.rows[0];
}

async function removeExpiredUnusedAutoContests() {
  await pool.query(
  ...
  for (const [game, config] of Object.entries(AUTO_GAMES)) {
    const result = await pool.query(
      `SELECT starts_at FROM contests
       WHERE game=$1 AND status='upcoming' AND starts_at > NOW()
    ...
    let nextStart;
    if (existing) {
      nextStart = new Date(result.rows[existing - 1].starts_at);
      nextStart = new Date(nextStart.getTime() + AUTO_MATCH_INTERVAL_MINUTES * 60000);
    } else {
      nextStart = new Date(Date.now() + config.startOffsetMinutes * 60000);
    }

    for (let i = 0; i < missing; i += 1) {
      await createAutoContest(game, config, nextStart.toISOString());
      nextStart = new Date(nextStart.getTime() + AUTO_MATCH_INTERVAL_MINUTES * 60000);
    }
  }
}
...
function defaultPrizeBreakup(game, prizePool, slots) {
  const count = resultPrizeCount(game, slots);
  if (!count || Number(prizePool) <= 0) return [];
  const total = Number(prizePool);
  const weights = [35,15,15];
  while (weights.length < count) weights.push(35 / Math.max(1, count - 3));
  const weightTotal = weights.slice(0,count).reduce((a,b)=>a+b,0);
  const values = weights.slice(0,count).map(w => Math.floor((total * w / weightTotal) * 100) / 100);
  let used = values.reduce((a,b)=>a+b,0);
  const remainder = Math.round((total - used) * 100) / 100;
  if (values.length) values[0] = Math.round((values[0] + remainder) * 100) / 100;
  return values.map((prize, i) => ({ rank: i + 1, prize }));
}
